import pygame
import sys

pygame.init()
WIDTH, HEIGHT = pygame.display.get_desktop_sizes()[0]
CENTER = pygame.Vector2(WIDTH // 2, HEIGHT // 2)
WINDOW_RECT = pygame.Rect(0, 0, WIDTH, HEIGHT)
FPS = 120
TITLE = "Spacetime Controller"
MY_WIDTH = 1536
WEB = False
if sys.platform in ("emscripten", "wasi"):
    WEB = True


def SCALE_RES(value):
    div = MY_WIDTH / value
    return max(1, int(WIDTH / div))


MOUSE_SPEED = 1500
CHUNK_SIZE = SCALE_RES(800)
UNIVERSE_SIZE = CHUNK_SIZE * 20
UNIVERSE_INFLATED = UNIVERSE_SIZE + CHUNK_SIZE * 2
UNIVERSE_RECT = pygame.Rect(0, 0, UNIVERSE_SIZE, UNIVERSE_SIZE).move_to(center=(0, 0))
STARS_IN_CHUNK = 50
DUSTS_IN_FAR_CHUNK = 8
if WEB:
    DUSTS_IN_FAR_CHUNK = 2
    STARS_IN_CHUNK = 20
DUST_FAR_CHUNK_SIZE_RANGE = (int(CHUNK_SIZE // 1.2), int(CHUNK_SIZE // 1))
DUST_CHANCE = 50
DUST_SIZE_RANGE = DUST_FAR_CHUNK_SIZE_RANGE
STAR_SIZE_RANGE = (2, 6)

MENU_STARS = 200
MENU_DUSTS = 4
MENU_DUST_SIZE_RANGE = (WIDTH / 3, WIDTH / 2)
MENU_PLAY_SPEED = 100
BTN_COL = (200, 120, 0)
BTN_HOVER = (255, 200, 0)
MENU_COMPLETED_COL = (0, 180, 0)
MENU_COMPLETED_HOVER = (0, 220, 0)

BUTTON_SEGMENT = SCALE_RES(16)
BUTTON_R = SCALE_RES(16)
BUTTON_W = 5
BUTTON_WS = 3
BUTTON_SPEED = 50

ASTEROID_HIT_SPEED = 380
ASTEROID_AMOUNT = 500
ASTEROID_HIT_COOLDOWN = 500
ASTEROID_POINTS_RANGE = (8, 12)
ASTEROID_SIZE_RANGE = (SCALE_RES(50), SCALE_RES(120))
ASTEROID_SPEED_RANGE = (5, 20)
ASTEROID_COL1 = pygame.Color(139, 69, 19)
ASTEROID_COL2 = pygame.Color(188, 143, 143)

RESOURCE_AMOUNT_RANGE = (2, 4)
RESOURCE_POINTS_RANGE = (5, 7)
RESOURCE_COOLDOWN = 20000
RESOURCE_ROT_SPEED_RANGE = (80, 120)
RESOURCE_COLLECT_SPEED = 15
RESOURCES = {
    "iron": ((110, 110, 110), 1),
    "quartz": ((255, 105, 180), 1),
    "ruby": ((255, 0, 0), 2),
    "gold": ((200, 180, 0), 2),
    "ametist": ((50, 0, 200), 3),
    "emerald": ((0, 200, 40), 3),
}
RESOURCE_COLID = 0
RESOURCE_HEALID = 1
RESOURCES_WEIGHTED = list(RESOURCES.keys())

FRAGMENT_SIZE_RANGE = (SCALE_RES(8), SCALE_RES(15))
FRAGMENT_SPEED_RANGE = (30, 60)
FRAGMENT_COOLDOWN = 5000

BLACKHOLE_AMOUNT = 12
BLACKHOLE_SIZE_RANGE = (300, 500)
BLACKHOLE_MULT = 1.6

DUST1_START = pygame.Color(255, 150, 0, 255)
DUST1_END = pygame.Color(255, 0, 255, 255)
DUST2_START = pygame.Color(255, 50, 0, 255)
DUST2_END = pygame.Color(255, 20, 255, 255)

PLAYER_SPEED = 500
PLAYER_ACC = 700
PLAYER_FRICTION = 850
PLAYER_ROT_SPEED = 10
PLAYER_SIZE = (SCALE_RES(60), SCALE_RES(40))
PLAYER_MAX_RESOURCES = 20
PLAYER_COL = "red"
PLAYER_HEALTH = 60
PLAYER_TRAIL_SPEED = 25
PLAYER_TRAIL_COL = (0, 100, 255)
PLAYER_TRAIL_COOLDOWN = 40
PLAYER_STRING_COL = (120, 120, 120)

ENEMY_PACK_COOLDOWN_START = 60000
ENEMY_PACK_COOLDOWN_DECREASE = 2000
ENEMY_PACK_COOLDOWN_MIN = 22000
ENEMY_ROT_SPEED = 3
ENEMIES = {
    "green": ["green", 5, (10, 15), 240, (SCALE_RES(50), SCALE_RES(30))],
    "pink": ["magenta", 8, (8, 12), 260, (SCALE_RES(60), SCALE_RES(40))],
    "yellow": ["yellow", 12, (6, 10), 290, (SCALE_RES(110), SCALE_RES(30))],
    "blue": ["blue", 15, (4, 8), 300, (SCALE_RES(90), SCALE_RES(50))],
    "red": ["red", 12, (6, 10), 295, (SCALE_RES(22), SCALE_RES(130))],
}
ENEMY_COLID = 0
ENEMY_DAMAGEID = 1
ENEMY_PACKSIZEID = 2
ENEMY_SPEEDID = 3
ENEMY_SIZEID = 4

SHIELD_ROT_SPEED = 200
SHIELD_PUSH_SPEED = 600
SHIELD_MULT = 1.3

WEAPON_DISAPPEAR_SPEED = 400
WEAPONS = {
    "red_hole": [CHUNK_SIZE / 4.5, CHUNK_SIZE / 4.5, 1500, 20, "ruby"],
    "purple_hole": [CHUNK_SIZE / 1.2, CHUNK_SIZE / 2, 6000, 10, "ametist"],
    "white_hole": [CHUNK_SIZE, CHUNK_SIZE / 3, 5000, 10, "iron"],
    "worm_hole": [CHUNK_SIZE * 8, CHUNK_SIZE / 4, 1500, 9, "emerald"],
    "supernova": [CHUNK_SIZE / 2.7, CHUNK_SIZE / 4, 1000, 8, "gold"],
    "shield": [CHUNK_SIZE / 1.8 / 2 / SHIELD_MULT, CHUNK_SIZE / 1.8, 8000, 8, "quartz"],
}
WEAPON_RADID = 0
WEAPON_SIZEID = 1
WEAPON_DURATIONID = 2
WEAPON_PRICEID = 3
WEAPON_RESOURCEID = 4

PURPLEHOLE_SUCK_SPEED = 800
WORMHOLE_ACC = 900

WHITEHOLE_SHOOT_COOLDOWN = 92
WHITEHOLE_PARTICLE_SPEED = 400
WHITEHOLE_PARTICLE_SIZE = SCALE_RES(150)

SUPERNOVA_START = pygame.Color(255, 200, 0)
SUPRNOVA_END = pygame.Color(100, 0, 0)
SUPERNOVA_EXPLOSION_TIME = 500
SUPERNOVA_COLS = [
    (255, 255, 0),
    (255, 200, 0),
    (255, 100, 0),
    (255, 0, 0),
    (255, 0, 30),
    (255, 0, 100),
    (255, 0, 180),
    (255, 0, 200),
    (200, 0, 255),
    (150, 0, 255),
    (80, 0, 255),
    (0, 0, 255),
    (0, 50, 255),
    (0, 120, 255),
    (0, 200, 255),
    (50, 220, 200),
    (50, 255, 100),
    (50, 255, 50),
]
SUPERNOVA_LAST = SUPERNOVA_COLS[-1]

EXPLOSION_SIZE = 120
EXPLOSION_TIME = 300
EXPLOSION_SECTION_SIZE = 30
EXPLOSION_GRAY = (80, 80, 80)
EXPLOSION_COLS = [
    (255, 255, 0),
    (255, 220, 0),
    (255, 200, 0),
    (255, 150, 0),
    (255, 100, 0),
    (255, 50, 0),
    (200, 0, 0),
    (150, 0, 0),
    (100, 0, 0),
]
EXPLOSION_LAST = EXPLOSION_COLS[-1]

UI_RESOURCE_SIZE = SCALE_RES(18)
UI_S = SCALE_RES(2)
UI_BG = (35, 35, 50)
UI_OUTLINE = (50, 50, 65)
UI_RESOURCE_DEFAULT = (20, 20, 35)
UI_BR = SCALE_RES(4)
UI_MAP_SIZE = SCALE_RES(160)
UI_MAX_RESOURCES = 200
UI_HEALTH_SIZE = (SCALE_RES(300), SCALE_RES(17))
UI_HEALTH_BG = (90, 0, 20)
UI_HEALTH_FILL = (180, 0, 50)
UI_HEALTH_OUTLINE = (220, 50, 80)
UI_WEAPON_SIZE = SCALE_RES(60)
UI_OK_COL = (0, 220, 50)
UI_BAD_COL = (220, 0, 50)
UI_FINISH_SPEED = 90
UI_OVERLAY_ALPHA = 200
UI_PAUSE_INCREASE = HEIGHT / 15

DIFFICULTIES = {"easy": 0, "normal": 1, "hard": 2, "extreme": 3}
DIFFICULTY_RESOURCES = {"easy": 300, "normal": 500, "hard": 700, "extreme": 1000}
